...
ERROR_GUILD_ID = os.getenv("DISCORD_GUILD_ID", "[channel-id]")
            ...
            for line in proxy_conf.read_text().splitlines():
                ...
                if line.startswith("http ") or line.startswith("https ") or line.startswith("socks4 ") or line.startswith("socks5 "):
                    ...
                    if len(parts) >= 3:
                        ...
                        # Proxy is handled by proxychains4 at library level (LD_PRELOAD)
                        # No need to set HTTPS_PROXY/HTTP_PROXY env vars
                        log.info(f"Proxy loaded: {ip}:{port} (proxychains4)")
                        return proxy_url
