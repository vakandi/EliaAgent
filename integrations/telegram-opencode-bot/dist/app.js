...
import { existsSync as existsSync29, readFileSync as readFileSync2 } from "fs";
var OpenCodeServerService = class {
  ...
  isProxyEnabled() {
    return existsSync29(join29(process.env.HOME || "", "EliaAI", ".proxy_enabled"));
  }
  loadProxyEnv() {
    const proxyConf = join29(process.env.HOME || "", ".proxychains.conf");
    if (!existsSync29(proxyConf)) return {};
    try {
      const content = readFileSync2(proxyConf, "utf-8");
  ...
  async startServer() {
    ...
    try {
      ...
      const finalArgs = useProxy ? ["-f", join29(process.env.HOME || "", ".proxychains.conf"), ...args] : args;
      const spawnEnv = useProxy ? { ...process.env, ...this.loadProxyEnv() } : process.env;
      this.serverProcess = spawn2(cmd, finalArgs, {
        detached: true,
        stdio: "ignore",
        env: spawnEnv
      });
