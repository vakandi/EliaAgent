    ...
    describe("#given config dir inside profiles/", () => {
      describe("#when getOpenCodeSkillDirs is called", () => {
        it("#then returns both profile and parent skill dirs", async () => {
          process.env.OPENCODE_CONFIG_DIR = "/path/to/config/opencode/profiles/opus"

          const { getOpenCodeSkillDirs } = await import("./opencode-command-dirs")
          const dirs = getOpenCodeSkillDirs({ binary: "opencode" })

          expect(dirs).toContain("/path/to/config/opencode/profiles/opus/skills")
          expect(dirs).toContain("/path/to/config/opencode/profiles/opus/skill")
          expect(dirs).toContain("/path/to/config/opencode/skill")
          expect(dirs).toContain("/path/to/config/opencode/skills")
    ...
    describe("#given config dir NOT inside profiles/", () => {
      describe("#when getOpenCodeSkillDirs is called", () => {
        it("#then returns only the config dir skills", async () => {
          process.env.OPENCODE_CONFIG_DIR = "/path/to/config/opencode"
    ...
    describe("#given config dir inside profiles/", () => {
      describe("#when getOpenCodeCommandDirs is called", () => {
        it("#then returns both profile and parent command dirs", async () => {
          ...
          expect(dirs).toContain("/path/to/config/opencode/profiles/opus/commands")
          expect(dirs).toContain("/path/to/config/opencode/profiles/opus/command")
          expect(dirs).toContain("/path/to/config/opencode/commands")
          expect(dirs).toContain("/path/to/config/opencode/command")
          expect(dirs).toHaveLength(4)
        })
      })
    })
