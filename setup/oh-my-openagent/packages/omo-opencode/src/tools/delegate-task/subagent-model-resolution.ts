...
export async function resolveSubagentModel(
  ...
  const effectiveAgentModel = agentOverride?.model ?? agentCategoryModel ?? agentConfigModel ?? matchedAgentModelStr

  if (agentOverride?.model || agentCategoryModel || agentRequirement || matchedAgent.model || agentConfigModel) {
    const resolution = resolveModelForDelegateTask({
      ...
      categoryDefaultModel: matchedAgentModelStr ?? agentConfigModel,
  ...
  if (!categoryModel && agentConfigModel) {
    const parsedConfigModel = normalizeModelFormat(agentConfigModel)
    if (parsedConfigModel) {
      const fullModel = `${parsedConfigModel.providerID}/${parsedConfigModel.modelID}`
      if (availableModels.size === 0 || fuzzyMatchModel(fullModel, availableModels, [parsedConfigModel.providerID])) {
        categoryModel = parsedConfigModel
        log("[delegate-task] Using agent config model as fallback", {
          agent: agentToUse,
          model: agentConfigModel,
        })
      } else {
        log("[delegate-task] Skipping unavailable agent config model", {
          agent: agentToUse,
          model: agentConfigModel,
        })
      }
