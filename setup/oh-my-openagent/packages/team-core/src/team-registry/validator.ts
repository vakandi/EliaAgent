...
const UNKNOWN_SUBAGENT_MESSAGE =
  "Agent '<name>' is hard-rejected and cannot join teams. Use delegate-task with subagent_type: '<name>' for read-only consultation instead."
...
export function validateMemberEligibility(member: Member): void {
  ...
  // Denylist pattern: unknown agents pass through (eligible by default).
  // Only hard-reject agents are blocked. Matches compiled plugin behavior.
