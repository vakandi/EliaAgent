...
function translateMemberError(
  ...
  if (input.kind === "subagent_type" || (!hasKind && hasSubagentType)) {
    ...
    if (typeof input.subagent_type !== "string") {
      return new MemberValidationError(
        `subagent_type must be a string, got '${subagentType}'.`,
        name,
        "invalid-subagent-type",
      )
    }
    const eligibility = agentEligibilityRegistry[subagentType]
    if (eligibility && eligibility.verdict === "hard-reject") {
      return new MemberValidationError(
        eligibility.rejectionMessage ?? `Agent '${subagentType}' is not eligible as a team member. Use delegate-task instead.`,
        name,
        "hard-rejected-subagent",
      )
    }
    // Unknown agents pass through — eligible by default (open-allowlist).
  }
