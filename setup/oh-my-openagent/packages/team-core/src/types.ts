 ...
 * Open-allowlist: unknown agents pass through; only hard-reject agents are blocked.
 */
...
export function parseMember(input: unknown): Member {
  ...
  if (raw.subagent_type !== undefined && typeof raw.subagent_type === "string") {
    const entry = AGENT_ELIGIBILITY_REGISTRY[raw.subagent_type as keyof typeof AGENT_ELIGIBILITY_REGISTRY]
