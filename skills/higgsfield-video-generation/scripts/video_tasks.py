#!/usr/bin/env python3
"""
Higgsfield Video Task Queue Manager
Usage: python3 video_tasks.py status|add|complete|fail|list
"""
...
QUEUE_FILE = Path(__file__).parent.parent.parent / "HiggsField.ai-API-Wraper" / "higgsfield_video_tasks.json"
OUTPUTS_DIR = Path(__file__).parent.parent.parent / "HiggsField.ai-API-Wraper" / "outputs"
...
def cmd_status():
    ...
    waiting = [t for t in tasks if t["status"] == "waiting_images"]
    generating = [t for t in tasks if t["status"] == "generating"]
    completed = [t for t in tasks if t["status"] == "completed"]
    failed = [t for t in tasks if t["status"] == "failed"]
    
    print(f"\n=== VIDEO TASK QUEUE ===")
    print(f"Waiting for images: {len(waiting)}")
    ...
    if waiting:
        print(f"\nWaiting for images:")
        for t in waiting:
            print(f"  - {t['task_id']}: needs {t.get('start_frame')} + {t.get('end_frame')}")
...
def cmd_add(script_name, start_frame, end_frame, prompt):
    data = load_queue()
    task_id = f"{script_name}_video"
...
if __name__ == "__main__":
    ...
    if cmd == "status":
        ...
        print("Usage: python3 video_tasks.py status|add|start|list|complete|fail")
