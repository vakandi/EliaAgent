...
This document describes the testing workflow for the Snapchat Auto Add Chrome extension, including how to load the extension, interact with it, register devices with the bene2luxe backend, and access logs.
...
Before testing, you need a valid master key from the bene2luxe admin panel:

1. Go to: `https://bene2luxe.com/admin?section=channels`
...
agent-browser --profile ~/.agent-browser-profile \
  --extension "/Users/vakandi/Documents/MultiSaasDeploy/snapchat_army/snapchat_ecommerce_chrome_extension" \
  open about:blank
```
...
4. **Fill in the Snapchat Account**:
   ...
   agent-browser fill "ref=e11" "cobou.dev"
   ```
...
curl -X POST "https://bene2luxe.com/api/snapchat-ext/register" \
  ...
  -d '{
    ...
    "browser": "chrome",
    "os": "MacIntel",
    "snapchat_account_hint": "cobou.dev"
  }'
```
...
- Check network connectivity to `https://bene2luxe.com`
