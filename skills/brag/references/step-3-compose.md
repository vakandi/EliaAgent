...
**Motion design quality gate** (from [motion-design.md](motion-design.md)):

- [ ] Zero `linear` easing — all tweens use `power3.out`, `power4.out`, `back.out`, `elastic.out`, or `power2.in`/`power3.in` for exits.
- [ ] No simultaneous reveals — sequential elements have stagger (0.08–0.20s).
- [ ] Hero text uses 2-phase entrance (blur/scale → settle, then breathing).
- [ ] At least one element has subtle breathing/pulse animation after entrance.
- [ ] Transitions match content energy (see tone-specific transition guidance).
- [ ] No text on screen for less than 0.8s (readability floor).
- [ ] Background has subtle texture (grain, gradient mesh, or noise).
- [ ] Depth created through scale hierarchy, z-index layering, or parallax.
