...
**Typography:** ALL CAPS or heavy mixed case. Full-bleed scenes. Large type. Significant scale. Use font-weight 700–900. Letter-spacing: -0.02em to -0.04em for impact.
...
**Transitions:** Use from the Hyperframes catalog — match transition to content energy:
- **Major reveal**: `light-leak` or `overexposure-burn` (warm, dramatic)
- **Rhythmic cut**: `glitch` or `chromatic-aberration` (edgy, modern)
- **Dramatic wipe**: `push-slide` with `power3.inOut` easing (classic, powerful)
- **Scale reveal**: `zoom-through` with `power3.in` → `power3.out` (depth, immersion)

**Animation rules (cinematic-specific):**
- Hero text: 2-phase entrance — `scale: 0.85 → 1` + `filter: blur(8px) → 0` over 0.7s `power3.out`, then subtle `scale: 1.0 → 1.02` breathing
- Feature text: `clipPath` reveal from left (`inset(0 100% 0 0)` → `inset(0 0% 0 0)`) over 0.6s `power3.out`
- Stats/numbers: `scale: 2 → 1` slam with `back.out(2)` easing, 0.4s
- Background: subtle `scale: 1.0 → 1.05` drift over full scene duration (parallax depth)
- Stagger: 0.12–0.18s between sequential elements

**Texture:** Add subtle film grain (opacity 0.03–0.05) and vignette (radial gradient overlay) for cinematic depth.

**When to use:** Anything with natural epic quality. "Fish Flight School" maps perfectly. Anything involving scale, nature, or grand claims. Products that deserve the blockbuster treatment.
