...
> Read `/path/to/EliaAI/context/TOOLS.md` for full command reference.
...
bash: /path/to/EliaAI/tools/get_ide_work.sh
```
...
# Groups: YOURCO PowerRangers, YOURBRAND BUSINESS, YOURVENTURES, personal contacts
...
python3 /path/to/EliaAI/tools/google_workspace.py list-events
python3 /path/to/EliaAI/tools/google_workspace.py list-tasks
```
...
python3 /path/to/EliaAI/tools/google_workspace.py create-task \
  "[Task Title]" \
  "Details about what needs to be done"

# 2B. Create calendar events with reminders for time-sensitive tasks
python3 /path/to/EliaAI/tools/google_workspace.py create-event \
  "HIGH PRIORITY: [Task]" \
...
- YOURCO PowerRangers (always)
- YOURBRAND BUSINESS (if yourbrand tasks)
- YOURVENTURES (if YOURVENTURES tasks)
...
**Business Context**:
- **ALWAYS READ**: `/path/to/EliaAI/memory/*`
