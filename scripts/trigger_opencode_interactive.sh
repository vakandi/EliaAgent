...
start_mcp_servers() {
    ...
    LOCAL_SERVERS=("telegram" "mcp-atlassian" "yourbrand_mcp" "discord-mcp" "gmail")
    ...
    for server in "${LOCAL_SERVERS[@]}"; do
        ...
        if grep -q "\"$server\":" "$MCP_CONFIG_FILE" 2>/dev/null; then
            ...
            case "$server" in
                "telegram")
                    cd /path/to/Documents/mcps_server/telegram-mcp-server 2>/dev/null && \
                ...
                "mcp-atlassian")
                    ...
                    JIRA_USERNAME="your.name@gmail.com" \
                    JIRA_API_TOKEN="[your-jira-api-token]" \
                    CONFLUENCE_URL="[your-confluence-url]" \
                    CONFLUENCE_USERNAME="[your-email]" \
                    CONFLUENCE_API_TOKEN="[your-confluence-api-token]" \
                    /home/user/.local/bin/uvx mcp-atlassian >> "$LOG_DIR/mcp_atlassian.log" 2>&1 &
                    ;;
                "yourbrand_mcp")
                    cd /path/to/Documents/mcps_server/yourbrand_mcp 2>/dev/null && \
                    BENELUXE_API_URL="https://yourbrand.com" \
                    .venv/bin/python main.py >> "$LOG_DIR/mcp_yourbrand.log" 2>&1 &
                    ;;
                "discord-mcp")
                    cd /path/to/Documents/mcps_server/discord_mcp_custom 2>/dev/null && \
                    WATSON_DISCORD_HEADLESS=1 \
                    .venv/bin/python mcp_server.py >> "$LOG_DIR/mcp_discord.log" 2>&1 &
                    ;;
                "whatsapp")
                    cd /path/to/Documents/mcps_server/whatsapp-mcp/whatsapp-mcp-server 2>/dev/null && \
                    WHATSAPP_API_BASE_URL=http://localhost:8080/api \
                    .venv/bin/python main.py >> "$LOG_DIR/mcp_whatsapp.log" 2>&1 &
                    ;;
                "gmail")
                    cd /path/to/Documents/mcps_server/gmail-mcp-server 2>/dev/null && \
                    GMAIL_OAUTH_PATH=/home/user/.gmail-mcp/gcp-oauth.keys.json \
...
start_codemem_viewer() {
    ...
    nohup node \
        /path/to/EliaAI/integrations/codemem/packages/cli/dist/index.js \
