...
get_ram_usage() {
    local total=0
    for pid in $(pgrep -f "opencode" 2>/dev/null || echo ""); do
        local mem=0
...
kill_all_opencode() {
    ...
    # Get all opencode PIDs, exclude current script PID
    local opencode_pids
    opencode_pids=($(pgrep -f "opencode" 2>/dev/null | grep -v "^$$\$" || echo ""))
    local killed=0
    for pid in "${opencode_pids[@]}"; do
        if [[ -f "/tmp/opencode-serve.pid" ]]; then
            local server_pid=$(cat /tmp/opencode-serve.pid 2>/dev/null || echo "")
            if [[ "$pid" == "$server_pid" ]]; then
                log "Skipping server managed by opencode-serve.sh (PID: $pid)"
                continue
            fi
        fi
...
kill_agents() {
    ...
    local kiro_pids
    kiro_pids=($(pgrep -f "kiro" 2>/dev/null || echo ""))
    if [[ ${#kiro_pids[@]} -gt 0 ]]; then
    ...
    local ohmy_pids
    ohmy_pids=($(pgrep -f "oh-my-opencode" 2>/dev/null || echo ""))
    if [[ ${#ohmy_pids[@]} -gt 0 ]]; then
    ...
    local fcm_pids
    fcm_pids=($(pgrep -f "free-coding-models" 2>/dev/null || echo ""))
    if [[ ${#fcm_pids[@]} -gt 0 ]]; then
...
show_status() {
    ...
    local remaining
    remaining=$(pgrep -f "opencode" 2>/dev/null || echo "")
    if [[ -n "$remaining" ]]; then
