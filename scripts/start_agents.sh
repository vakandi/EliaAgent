...
if [[ -n "$EXTRA_PROMPT" ]]; then
    ...
    # Write the extra context to the temporary file
    cat > "$TIMESTAMPED_PROMPT" << EOF
# 🚨 URGENT CONTEXT - $(date '+%Y-%m-%d %H:%M:%S')

$EXTRA_PROMPT

EOF
    
    # Append the original PROMPT.md content
    if [[ -f "$PROMPT_FILE" ]]; then
        cat "$PROMPT_FILE" >> "$TIMESTAMPED_PROMPT"
    fi
...
echo "=========================================="
echo "[DEBUG] start_agents.sh - Ralph Mode Check"
