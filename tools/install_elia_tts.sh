...
#!/bin/bash
python3 /path/to/EliaAI/setup/speak.py "$@"
EOF

cat > /usr/local/bin/elia-voxtral-speak << 'EOF'
#!/bin/bash
python3 /path/to/Documents/mcps_server/dia_voice/mistral-speak.py "$@"
EOF

chmod +x /usr/local/bin/elia-speak /usr/local/bin/elia-voxtral-speak

echo "✅ Done! Run:"
echo "  sudo bash /path/to/EliaAI/tools/install_elia_tts.sh"