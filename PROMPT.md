...
| **REPORT** results to YourName | Aggregate and communicate outcomes |
...
task(category="yourbrand", load_skills=["brand-guidelines"], prompt="Create product listing. Context: @context/business.md")
```
...
| `yourbrand` | Luxury e-commerce tasks |
| `yourco-agency` | B2B digital tasks |
| `yourtool` | SMMPanel tasks |
...
1. **Lis** `/path/to/EliaAI/context/TOOLS.md` pour connaître la commande exacte
...
│   ├── YES → Discord DM to YourName
│   │   └── "Hey, le compte Stripe pour B2L, ça avance? On a 6000€ bloqués."
│   │
├── SSL (YourProject/YourBrand2 - HTTPS fail)?
│   ├── YES → WhatsApp to YourName
│   │   └── "YourName, les certificats SSL, c'est bon quand? HTTPS fail."
│   │
├── Payment (YourCo)?
│   ├── YES → WhatsApp YOURCO
│   │   └── "[Status], on avance?"
...
**Stripe (Discord DM to YourName — URGENT)**
```
"Hey, le compte Stripe pour B2L, ça avance? On a ~6000€ bloqués dessus. Besoin d'aide?"
```

**SSL (WhatsApp to YourName)**
```
"YourName, les certificats SSL pour YourProject et YourBrand2 sont expirés (HTTPS fail). C'est bon quand tu peux faire le renew?"
```
...
| `@memory/MEMORY-YOURBRAND-CREDENTIALS.md` | Credentials (B2L only) |
...
1. **Determine what to update**:
   ...
   - Credentials → @memory/MEMORY-YOURBRAND-CREDENTIALS.md  
   - Business → @context/business.md
   - Tools/commands → @context/TOOLS.md

2. **Read current content first**:
   - `read /path/to/EliaAI/memory/MEMORY.md`
   - `read /path/to/EliaAI/context/business.md`
   - etc.
...
| Credentials change | @memory/MEMORY-YOURBRAND-CREDENTIALS.md |
