...
> **Version:** 3.0 (Master - Avril 2026)  
> **Pour:** Client débutant dans le business du frontend premium  
...
PICASSO est un **agent IA spécialisé en développement frontend premium** créé pour OpenCode. Il combine:

- 🎨 **Design haut de gamme** - React, TypeScript, Tailwind, Framer Motion
- 💰 **Mentalité business** - conversion, trust, UX, rentabilité
...
| "Does this push the user to act?" | L'action génère le revenu |

### Le Système de Priorités

| Priority | Focus | Raison |
|----------|-------|--------|
| **1. CONVERSION** | Est-ce que ça pousse l'utilisateur à agir ? | Survie du business |
...
├── oh-my-opencode.json      # Config agent
└── config.json             # Config MCPs
```

### Étape 3: Configuration OpenCode

Éditez `~/.config/opencode/oh-my-opencode.json` et ajoutez:
...
| 3 | **premium-component-library** | Hero, pricing, features, testimonials, CTA, nav |
...
| # | Skill | Utilisation |
|---|-------|-------------|
| 1 | **ecommerce-funnel** | Modèle AIDA, value proposition, urgence |
| 2 | **cta-strategies** | Psychologie des boutons, copy, placement |
...
├── Analyse Structurelle (hiérarchie HTML/JSX)
...
├── Analyse du Contexte (site existant? brand?)
...
|------|----------|-----------|
| CTA | "Qu'est-ce que j'OBTIENS ?" | Action |
...
|-----------|---------------|------------------|
| "C'est fiable ?" | Sécurité | Badges SSL, social proof |
| "Est-ce authentique ?" | Preuve | Badge authenticité |
| "Et si je regrette ?" | Garantie | Garantie visible près CTA |
| "Et si la taille ?" | Flexibilité | Guide taille, retours gratuits |
| "Pourquoi ce prix ?" | Valeur | Value framing |
...
**Stratégie de chargement d'images:**
...
### Version A: Premium Éditorial
- Hero avec video backgrounds
- Images pleine largeur
- Copy émotionnel

### Version B: Conversion Directe
- Hero avec CTA bold
- Product cards above fold
- Copy transactionnel

### Hypothèse
"Version B convertira +15% car l'CTA est plus visible"
...
|---------|-------|---------|
| CTA Click | `cta_click` | Text, position, variant |
| Form Start | `form_start` | Step number |
| Form Complete | `form_complete` | Time, field count |
| Product View | `product_view` | Product ID, category |
| Add to Cart | `add_to_cart` | Product ID, price |
| Checkout Start | `checkout_start` | Cart value |

**Funnel Visibility Map:**
```
[Hero] → [Product] → [Cart] → [Checkout] → [Payment] → [Confirm]
   ↓          ↓           ↓         ↓            ↓            ↓
  100%       60%         40%       30%          20%          15%
            ↑
         Drop-off principal
...
| Manque trust | Pas de badges sécurité | Ajouter SSL, garanties |
| Étapes inutiles | Account creation forcé | Guest checkout first |
| Mauvais ordre | Email avant shipping | Shipping avant payment |
| CTA faible | "Soumettre" | "Confirmer ma commande" |
...
|---------|----------------|----------|
| Hero | TOUJOURS - capture | Attention + CTA |
| Social Proof (logos) | Early | Confiance immédiate |
| Features | Detail needed | Compréhension |
| Testimonials | Validation | Preuve sociale |
| Pricing | User prêt | Décision |
| FAQ | Avant CTA final | Éliminer objections |
| CTA | Fin | Conversion finale |
...
15. **Ne suppose pas que l'user sait faire le setup technique**
...
19. **MANDATORY: UX copywriting sur chaque CTA, label, error, empty state**
...
23. **MANDATORY: Sections stratégiques (Hero, Social Proof, CTA)**
...
| 🔀 Git | `clone`, `add`, `commit`, `push`, `pull`, `checkout` |
| 🏗️ Build | `npm run`, `vite`, `next`, `astro`, `build`, `dev` |
| 🎨 Frontend | `uipro`, `playwright`, `tailwindcss` |
| ✅ Quality | `eslint`, `prettier`, `lint` |
| 🧪 Test | `playwright test`, `npm run test` |
...
| 🛒 **E-commerce** | Product cards, cart UI, checkout flow |
...
- [ ] Créer la structure de dossiers `~/.config/opencode/`
- [ ] Copier `picasso.md` dans `~/.config/opencode/agents/`
- [ ] Copier tous les skills dans `~/.config/opencode/skills/`
- [ ] Configurer `oh-my-opencode.json` avec les skills PICASSO
- [ ] Configurer `config.json` pour les MCPs
...
```
USER: Crée une landing page e-commerce premium pour un produit luxury
...
3. **Lisez les docs** - Chaque skill a sa documentation détaillée
...
☐ J'ai créé la structure de dossiers  
☐ J'ai copié tous les fichiers PICASSO  
☐ J'ai configuré OpenCode  
☐ J'ai installé les dépendances  
☐ J'ai activé PICASSO avec `/agent picasso`  
☐ J'ai compris les 3 priorités (Conversion, UX, Design)  
...
*Pour un client.noobie dans le business du frontend premium*  
